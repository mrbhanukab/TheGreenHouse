...
//? valid status screen
void validStatusUserScreeen(byte status)
{
    display.fillRect(0, 0, SCREEN_WIDTH, 50, SSD1306_BLACK);
    display.setTextSize(1);
    display.setCursor(40, 20);
...
void bottomPanel(String ssid, String ip)
{
    unsigned long currentMillis = millis();
    ...
    if (currentMillis - previousMillis >= 3500)
    {
        ...
        showSSID = !showSSID; // Toggle between SSID and IP
    }
...
void bootingSplash(bool isBooted)
{
    display.clearDisplay();
    ...
    if (isBooted)
    {
        display.setCursor((SCREEN_WIDTH - 54) / 2, 50);
        display.print("All Good!");
    }
    else
    {

        display.setCursor((SCREEN_WIDTH - 66) / 2, 50);
